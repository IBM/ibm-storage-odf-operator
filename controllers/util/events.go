...
const (
	...
	FailedFscConfigMapReason            = "FailedEnsureFscConfigMap"
	FailedPoolsConfigMapReason          = "FailedEnsurePoolsConfigMap"
	...
	PoolOGChangeReason                  = "PoolOwnershipChange"
)
...
func InitK8sPoolOGChangeEvent(instance *odfv1alpha1.FlashSystemCluster, poolName, ogName string) *corev1.Event {
	t := metav1.Time{Time: time.Now()}
	selectLabels := GetLabels()
	return &corev1.Event{
		ObjectMeta: metav1.ObjectMeta{
			Name:      fmt.Sprintf("PoolOwnershipChange_for_%v", poolName),
			Namespace: instance.Namespace,
			Labels:    selectLabels,
		},
		InvolvedObject: corev1.ObjectReference{
		...
		Message:        fmt.Sprintf("Pool %v changed ownership to %v", poolName, ogName),
		FirstTimestamp: t,
		LastTimestamp:  t,
		Count:          1,
