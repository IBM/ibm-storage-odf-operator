...
func GetCreateConfigmap(client client.Client, log logr.Logger, ns string, createIfMissing bool) (*corev1.ConfigMap, error) {
	...
	if err != nil && createIfMissing {
		if errors.IsNotFound(err) {
			...
			log.Info("Creating", "ConfigMap", configMap.Name)
			if err != nil {
				log.Error(err, "Failed to create", "ConfigMap", PoolConfigmapName)
				return nil, err
			}
		} else {
			log.Error(err, "Failed to get", "ConfigMap", PoolConfigmapName)
			return nil, err
		}
	}
