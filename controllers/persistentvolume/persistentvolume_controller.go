...
func (r *StorageClassWatcher) getFscByManagementAddress(mgmtAddr string) (v1alpha1.FlashSystemCluster, error) {
	clusters := &v1alpha1.FlashSystemClusterList{}
	if err := r.Client.List(context.Background(), clusters); err != nil {
		r.Log.Error(nil, "failed to list FlashSystemClusterList")
		return v1alpha1.FlashSystemCluster{}, err
	}
	...
	for _, c := range clusters.Items {
		...
		clusterMgmtAddr := string(clusterSecret.Data[util.SecretManagementAddressKey])

		if mgmtAddr == clusterMgmtAddr {
			r.Log.Info("found FlashSystemCluster for the given management address")
			return c, nil
		}
	}
	msg := "failed to match FlashSystemCluster to the given management address"
	r.Log.Error(nil, msg)
	return v1alpha1.FlashSystemCluster{}, fmt.Errorf(msg)
}
